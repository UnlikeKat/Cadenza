...
export class OpenSheetMusicDisplay {
    private version: string = "1.9.2-audio-extended"; // getter: this.Version
    ...
    private isMxl (data: string): boolean {
        if (data.startsWith("\x50\x4b\x03\x04")) {
            return true;
        }
        return false;
    }
    ...
    private processStringXml (xmlString: string): Document {
        ...
        if (xmlString.startsWith("\uf7ef\uf7bb\uf7bf")) {
            ...
            return parser.parseFromString(xmlString.substring(3), "application/xml");
        }
    ...
    public load(content: string | Document, tempTitle: string = "Untitled Score"): Promise<{}> {
        ...
        const loadPromise: Promise<any> = new Promise(
            ...
            contentPromise.then(function(musicXML: Document): void {
                ...
                const reader: MusicSheetReader = new MusicSheetReader([dynamicsGenerator, temposCalculator, playbackNoteGen], self.rules);
    ...
    public render(): void {
        ...
        if (this.rules.RenderSingleHorizontalStaffline) {
            ...
            this.renderingManager.MainViewingRegion.WidthInUnits = this.container.offsetWidth / this.zoom / 10.0;
        }
