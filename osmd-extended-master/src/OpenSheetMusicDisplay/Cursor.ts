...
export class Cursor implements IPlaybackListener {
  ...
  public soundLoaded(instrumentId: number): void {
    // throw new Error("Method not implemented.");
  }
  public allSoundsLoaded(): void {
    // throw new Error("Method not implemented.");
  }
  public metronomeSoundOccurred(o: MetronomeSoundOccuredData): void {
    // console.log("metronome sound occurred, volume: " + o.volume + ", firstBeat: " + o.firstBeat);
    // do nothing
  }
  ...
  public updateWithTimestamp(timestamp: Fraction): void {
    ...
    const stayAtLeftStaffEntry: boolean = !this.rules.InterpolateCursorPositionBetweenNotesDuringPlayback;
    const values: [number, MusicSystem, GraphicalStaffEntry] = this.graphic.calculateXPositionFromTimestamp(sheetTimestamp, stayAtLeftStaffEntry);
    ...
    //   so [topMeasureNumber - 1] would be [-1], causing an error
    ...
    const topMeasure: GraphicalMeasure = this.graphic.findGraphicalMeasureByMeasureNumber(topMeasureNumber, 0);
