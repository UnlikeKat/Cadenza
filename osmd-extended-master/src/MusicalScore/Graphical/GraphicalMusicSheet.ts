...
export class GraphicalMusicSheet {
    ...
    public GetNearestVoiceEntry(clickPosition: PointF2D, ignoreGraceNotes: boolean = false): GraphicalVoiceEntry {
        function shouldBeIncluded(gve: GraphicalVoiceEntry): boolean {
            if (!gve.parentStaffEntry?.relInMeasureTimestamp) {
                return false;
            }
            if (ignoreGraceNotes) {
    ...
    public calculateXPositionFromTimestamp(timeStamp: Fraction, stayAtLeftStaffEntry: boolean = false): [number, MusicSystem, GraphicalStaffEntry] {
        ...
        if (previousStaffEntryMusicSystem === nextStaffEntryMusicSystem) {
            ...
            if (stayAtLeftStaffEntry) {
                return [previousStaffEntryPositionX, currentMusicSystem, previousStaffEntry];
            }
